...
# Re-define old and new requirements for testing

ORDER_SENSITIVE_PATTERNS = {
    "control_flow": {
        "A_followed_by_B", "A_precedes_B", "A_immediately_followed_by_B", "A_blocks_B_until_C"
    },
    "time": {
        "time_lag", "duration", "schedule_restricted"
    }
}
    ...
    for old in old_list:
        ...
        for new in new_list:
            if id(new) in matched_new_ids:
                continue
            # Matching rules...
            if old.get("element") == new.get("element") or old.get("activities") == new.get("activities"):
                best_match = new
                break
            if old.get("dimension") == new.get("dimension") and old.get("compliance_pattern") == new.get(
                    "compliance_pattern"):
                best_match = new
                break

        if best_match:
            matched_new_ids.add(id(best_match))  # store by id
...
def compare_actions(old_action, new_action):
    ...
    if old_action.get("dimension") != new_action.get("dimension"):
        changes.append({"change": "modify", "element": "dimension", "old": old_action.get("dimension"), "new": new_action.get("dimension")})

    for field in ["activities", "resources", "data_values", "temporal_constraints"]:
        if old_action.get(field) != new_action.get(field):
            changes.append({"change": f"modify_{field}", "element": field, "old": old_action.get(field), "new": new_action.get(field)})
    return changes
